import { Download, FileSpreadsheet, Plus, RefreshCw, Search, Wrench, XCircle, Activity, Package } from 'lucide-react'
...
function DevicesHeader({ categories, filters, onAddDevice, onFilterChange, onRefresh, searchValue, setSearchValue, stats, statuses }) {
  const statCards = [
    { key: 'total', label: 'Tổng số thiết bị', value: stats.total, color: 'bg-blue-50 text-[#2563EB]' },
    { key: 'active', label: 'Thiết bị đang hoạt động', value: stats.active, color: 'bg-emerald-50 text-emerald-700' },
    { key: 'maintenance', label: 'Thiết bị đang bảo trì', value: stats.maintenance, color: 'bg-amber-50 text-amber-700' },
    ...
    <header className="grid min-w-0 gap-4">
      <div className="flex min-w-0 flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#2563EB] sm:text-sm">Danh mục dùng chung</p>
          <h1 className="mt-2 break-words text-2xl font-bold tracking-tight text-slate-950 min-[420px]:text-3xl sm:text-4xl">Quản lý thiết bị phòng họp</h1>
        ...
        <div className="grid w-full min-w-0 grid-cols-1 gap-2 min-[420px]:grid-cols-3 xl:w-auto">
          <button className="inline-flex h-11 min-w-0 items-center justify-center gap-2 rounded-2xl bg-[#2563EB] px-4 text-sm font-bold text-white shadow-sm transition hover:bg-blue-700" onClick={onAddDevice} type="button">
      ...
      <section className="grid min-w-0 gap-3 rounded-2xl border border-gray-300 bg-white p-4 shadow-sm sm:p-5 xl:grid-cols-[minmax(240px,1.4fr)_minmax(170px,1fr)_minmax(170px,1fr)_auto]">
        <label className="grid min-w-0 gap-1.5">
          ...
          <span className="flex h-11 min-w-0 items-center gap-2 rounded-2xl border border-gray-300 bg-white px-3 transition focus-within:border-[#2563EB] focus-within:ring-4 focus-within:ring-blue-100">
            ...
            <input className="min-w-0 flex-1 bg-transparent text-sm font-semibold text-slate-700 outline-none placeholder:text-slate-400" onChange={(event) => setSearchValue(event.target.value)} placeholder="Tìm theo tên thiết bị..." value={searchValue} />
          </span>
        </label>
        <FilterSelect label="Loại thiết bị" name="category" onChange={onFilterChange} options={categories} value={filters.category} />
