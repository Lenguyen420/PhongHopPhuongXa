...
function UserModal({ departments, mode, onClose, onDeleteUser, onSaveUser, rolePermissions, roles, statuses, user }) {
  ...
  const togglePermission = (permission) => {
    setPermissions((currentPermissions) => (currentPermissions.includes(permission) ? currentPermissions.filter((item) => item !== permission) : [...currentPermissions, permission]))
  }

  const handleSave = () => {
    onSaveUser({
      ...formData,
      avatar: formData.name.split(' ').slice(-2).map((part) => part[0]).join('').toUpperCase() || 'ND',
      permissions,
    })
  }
      ...
      <section className="max-h-[calc(100vh-24px)] w-full max-w-4xl overflow-y-auto rounded-2xl border border-gray-300 bg-white shadow-2xl sm:max-h-[calc(100vh-32px)]">
        <div className="flex min-w-0 items-start justify-between gap-3 border-b border-gray-300 px-4 py-4 sm:px-5">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">{user?.name || 'Tài khoản mới trong hệ thống họp trực tuyến UBND phường/xã'}</p>
          </div>
        ...
        <div className="p-4 sm:p-5">
          ...
          {isFormMode && <UserForm departments={departments} formData={formData} mode={mode} onChange={handleChange} roles={editableRoles} statuses={statuses} />}
          {mode === 'permissions' && <PermissionPanel permissions={permissions} rolePermissions={rolePermissions} selectedRole={selectedPermissionRole} togglePermission={togglePermission} />}
          {mode === 'reset' && <ConfirmPanel icon={KeyRound} text="Mật khẩu mới sẽ được tạo và gửi đến email công vụ của người dùng." tone="blue" />}
          {mode === 'lock' && <ConfirmPanel icon={Lock} text="Tài khoản sẽ không thể đăng nhập cho đến khi được mở khóa lại." tone="amber" />}
          {mode === 'delete' && <ConfirmPanel icon={Trash2} text="Người dùng sẽ bị xóa khỏi danh sách mẫu trên giao diện hiện tại." tone="red" />}
        </div>
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-gray-300 bg-slate-50 px-4 py-4 sm:flex-row sm:justify-end sm:px-5">
          ...
          {mode === 'delete' ? (
            <button className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-red-600 px-5 text-sm font-bold text-white transition hover:bg-red-700" onClick={() => onDeleteUser(user.id)} type="button">
              <Trash2 size={17} />
              Xóa
            </button>
          ) : (
            <button className={`inline-flex h-11 items-center justify-center gap-2 rounded-2xl px-5 text-sm font-bold text-white transition ${isDangerMode ? 'bg-amber-600 hover:bg-amber-700' : 'bg-[#2563EB] hover:bg-blue-700'}`} onClick={isFormMode || mode === 'permissions' ? handleSave : handleConfirm} type="button">
              <Check size={17} />
              Lưu
            </button>
          )}
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[220px_minmax(0,1fr)]">
      <div className="grid place-items-center rounded-2xl border border-gray-300 bg-slate-50 p-5">
        <span className="grid h-24 w-24 place-items-center rounded-2xl bg-blue-50 text-3xl font-bold text-[#2563EB]">{user.avatar}</span>
...
function UserForm({ departments, formData, mode, onChange, roles, statuses }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <FormInput label="Email" name="email" onChange={onChange} type="email" value={formData.email} />
      ...
      <FormSelect label="Đơn vị" name="department" onChange={onChange} options={departments} value={formData.department} />
      <FormSelect label="Vai trò" name="role" onChange={onChange} options={roles} value={formData.role} />
      <FormSelect label="Trạng thái" name="status" onChange={onChange} options={statuses} value={formData.status} />
      {mode === 'create' && <FormInput label="Mật khẩu" name="password" onChange={onChange} type="password" value={formData.password} />}
    </div>
  )
}
  ...
  return (
    <div className="grid gap-4">
      {Object.entries(rolePermissions).map(([role, items]) => (
        <section className={`rounded-2xl border p-4 ${role === selectedRole ? 'border-blue-200 bg-blue-50/50' : 'border-gray-300 bg-white'}`} key={role}>
          <div className="flex items-center gap-2">
            <ShieldCheck className={role === selectedRole ? 'text-[#2563EB]' : 'text-slate-400'} size={18} />
            <h3 className="text-base font-bold text-slate-950">{role}</h3>
          </div>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {items.map((permission) => (
              <label className="flex min-w-0 items-center gap-3 rounded-2xl border border-gray-300 bg-white px-3 py-3 text-sm font-bold text-slate-700" key={permission}>
                <input checked={permissions.includes(permission)} className="h-4 w-4 accent-[#2563EB]" onChange={() => togglePermission(permission)} type="checkbox" />
...
function ConfirmPanel({ icon: Icon, text, tone }) {
  const toneClass = tone === 'red' ? 'bg-red-50 text-red-600' : tone === 'amber' ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-[#2563EB]'
...
function FormInput({ label, name, onChange, type = 'text', value }) {
  return (
    <label className="grid min-w-0 gap-1.5">
      ...
      <input className="h-11 min-w-0 rounded-2xl border border-gray-300 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-[#2563EB] focus:ring-4 focus:ring-blue-100" onChange={(event) => onChange(name, event.target.value)} type={type} value={value} />
    </label>
  )
}
